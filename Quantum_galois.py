...
class TestQuantumFieldExtensionCalculator(unittest.TestCase):
    ...
    def test_automorphisms(self):
        ...
        # For x^2 - 2, there should be 2 automorphisms (identity and mapping sqrt(2) to -sqrt(2))
        self.assertEqual(len(automorphisms), 2)
        
        # Check that we have the correct number of mappings
        self.assertEqual(len(automorphisms[0]), 2)
    
    def test_subgroup_computation(self):
        """Test computation of subgroups of a Galois group."""
        galois_solver = QuantumGaloisGroupSolver()
        poly = sp.Poly(self.x**3 - 2, self.x)
        galois_group = galois_solver.compute_galois_group(poly)
        
        subgroups = self.calculator._get_subgroups(galois_group)
        
        # S3 has 4 subgroups: trivial, 3 of order 2, and 1 of order 3
        # Our simplified implementation might not get all of them
